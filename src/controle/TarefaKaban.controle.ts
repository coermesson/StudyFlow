...
export class TarefaKanbanControle {
 ...
 /* public async atualizar(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const { titulo, descricao, status, prioridade, quadroId } = req.body;

      const atualizado = await this.tarefaServico.atualizar({
        id, titulo, descricao, status, prioridade, quadroId
      });
